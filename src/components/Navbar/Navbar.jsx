...
const Navbar = () => {
    ...
    return <div className="shadow-md w-full fixed top-0 left-0 m-auto">
        <nav className="sm:flex bg-white py-4 items-center justify-between">
            <div className="flex font-bold text-2xl cursor-pointer items-center font-display gap-2 sm:px-10 px-7 text-cyan-950 group">
                    <svg className="group-hover:fill-violet-500 duration-300" fill="#083344" width="32px" height="32px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="group-hover:text-violet-500 duration-300">Semaninha</span>
            </div>

            <div onClick={()=>setOpen(!open)}className="text-3xl absolute right-8 top-4 cursor-pointer sm:hidden text-cyan-950 hover:text-violet-500 duration-300">
                <ion-icon name={open ? 'close':'menu'}></ion-icon>
            </div>
            <ul className={`sm:flex sm:items-center sm:gap-5 sm:pb-0 pb-12 absolute sm:static bg-white sm:z-auto z-[-1] left-0 w-full sm:w-auto sm:pl-0 pl-9 shadow-lg sm:shadow-none transition-all duration-500 ease-in ${open ? 'top-16 opacity-100':'top-[-490px]'} sm:opacity-100 sm:pr-10`}>
